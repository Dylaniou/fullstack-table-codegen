<template>
  <div class="${kebabCaseName}-container">
    <h2>${className} Management</h2>
    ...
    <div class="table-container">
      <table class="data-table">
        ...
        <tbody>
          <tr v-for="item in items" :key="item.${primaryKeyField}">
            <td>
              <input 
                type="checkbox" 
                v-model="selectedIds"
            ...
            <td class="action-buttons">
              ...
              <button @click="deleteItem(item.${primaryKeyField})" class="btn-delete">
                Delete
              </button>
            </td>
          </tr>
    ...
    <div v-if="showAddForm || showEditForm" class="modal-overlay" @click="closeModal">
      <div class="modal-content" @click.stop>
        <div class="modal-header">
          <h3>{{ showEditForm ? 'Edit' : 'Add New' }} ${className}</h3>
...
export default {
  ...
  methods: {
    ...
    editItem(item) {
      this.formData = { ...item };
      this.editingId = item.${primaryKeyField};
    ...
    async exportData() {
      try {
        ...
        link.href = url;
        link.setAttribute('download', '${kebabCaseName}-export.xlsx');
    ...
    handleSelectAll() {
      if (this.selectAll && Array.isArray(this.items)) {
        // 全选
        this.selectedIds = this.items.map(item => item.${primaryKeyField});
      } else {
        // 取消全选
        this.selectedIds = [];
      }  
